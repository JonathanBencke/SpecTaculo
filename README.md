...
- Python 3.11+
- PowerShell 5.1+ (Windows) ou PowerShell Core (Linux/macOS)
- Git

### Instalação rápida via `irm` (recomendada)
...
|-----------|-----------|---------|
| `-Tool` | CLI alvo: `claude`, `kimi`, `opencode`, `kiro` ou `all` | `-Tool all` |
| `-Target` | Diretório de destino (padrão: atual) | `-Target ..\meu-projeto` |
| `-Generate` | Regenera os artefatos antes de instalar | `-Generate` |
...
│   ├── templates/        # Templates Jinja2 por CLI
...
2. Siga o schema: `name`, `description`, `entry`, `output`, `rules`, `prompt`.
3. Regenere os artefatos com `python src/generate.py all --clean`.

### Customizar templates

Edite os arquivos em `src/templates/` para ajustar o formato de saída de cada CLI.
